...
/*   Updated: 2023/02/20 02:03:46 by ttakami          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	operation_rra(t_stk_info *in)
{
	...
	if (!in->a->top->next)
		return (1);
	if (!operationlist_add_back(in, R_ROTATE_A))
		return (0);
	new_top = in->a->bottom;
...
int	operation_rrb(t_stk_info *in)
{
	...
	if (!in->b->top->next)
		return (1);
	if (!operationlist_add_back(in, R_ROTATE_B))
		return (0);
	new_top = in->b->bottom;
...
int	operation_rrr(t_stk_info *in)
{
	...
	if (!in->a->top->next || !in->b->top->next)
		return (1);
	if (!operationlist_add_back(in, R_ROTATE_BOTH))
		return (0);
	new_top = in->a->bottom;
	in->a->bottom = in->a->bottom->prev;
	in->a->bottom->next = NULL;
